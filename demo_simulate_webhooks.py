...
class VajraDemoClient:
    ...
    async def detect_fraud_rings(self):
        ...
        resp = await self.client.post(f"{self.base_url}/fraud/detect-rings", json={"min_size": 3, "min_density": 0.2})
        return resp.json()
...
async def run_scenario_2_suspicious_transaction(client: VajraDemoClient):
    """Scenario 2: Suspicious transaction → high fraud score + ring detection"""
    print("\n" + "="*60)
...
async def run_scenario_4_fraud_ring_detection(client: VajraDemoClient):
    """Scenario 4: Proactive fraud ring detection"""
    print("\n" + "="*60)
    print("SCENARIO 4: Proactive Fraud Ring Detection")
    print("="*60)

    print("\n🔍 Scanning graph for fraud rings...")
    result = await client.detect_fraud_rings()
    print(json.dumps(result, indent=2, default=str))
    ...
    if result.get("rings"):
        for ring in result["rings"]:
            ...
            print(f"   Entities: {len(ring['entities'])}")
